...
app.route('/profile')
  .get((req : Request , res : Response) => {
  ...
  }).patch(validationMiddleware(updateUserDto)  ,async (req : Request, res : Response) => {
    ...
    const updatedUser = await Users.findByIdAndUpdate(user._id , updatedAttributes , {
...
app.get('/all' , async (req : Request , res : Response) => {
  ...
  const otherUsers = users.filter(user => user.id.toString() !== req.user?._id);
...
app.get('/conversations' , async (req : Request , res : Response) => {
  ...
  for(let room of joined_rooms) {
    const last_msg = await MessageModel.findOne({room : room._id}).sort({timestamp : -1}).limit(1);
    messages.push({
      room,
      last_msg,
    });
